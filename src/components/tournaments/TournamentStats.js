 ...
 *
 * Props:
 *   tournament — full tournament object (with team_count, match_count etc.)
 */
export default function TournamentStats({ tournament }) {
  ...
  const upcoming_matches = match_count - completed_matches - live_matches;
  ...
  const stats = [
    ...
    },
    {
      label:     'Live Now',
      value:     live_matches,
      caption:   live_matches > 0 ? 'In progress' : 'No live games',
    ...
    {
      label:     'Upcoming',
      value:     Math.max(0, upcoming_matches),
      caption:   'Scheduled',
